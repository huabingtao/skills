...
基于 **social-auto-upload (Patchright 引擎)**，将 `article-to-img-skill` 生成的 3:4 切图集以及攻略元数据自动上传至 **B站 (哔哩哔哩) 创作者中心草稿箱**。
...
| 字段 | 来源 | 说明 |
| :--- | :--- | :--- |
| 标题 | `social_title` → `title` | B站专栏/图文标题 |
...
## Cookie / Profile 持久化 (集中式)

- 首次运行 `--login` 打开有头浏览器扫码登录
- Session Profile 统一集中保存至 `deps/social-auto-upload/cookies/bilibili_uploader/profile`
- 后续静默上传自动加载 Profile 并存草稿箱
